...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 현재 비밀번호가 맞는지 확인
    $checkPasswordSql = "SELECT youPass FROM myMembers WHERE memberId = '$memberId'";
    ...
    if ($result->num_rows > 0) {
        ...
        if ($currentPassword === $currentPasswordFromDB) {
            ...
            if (strlen($newPassword) < 8 || strlen($newPassword) > 20 || !$newPasswordNum || !$newPasswordEng || !$newPasswordSpe) {
                ...
                $updatePasswordSql = "UPDATE myMembers SET youPass = '$newPassword' WHERE memberId = '$memberId'";
