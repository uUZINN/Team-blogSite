...
// 기본 SQL 쿼리
$sql = "SELECT b.blogId, b.blogTitle, b.blogContents, m.youName, b.blogRegTime, b.blogView, b.blogLike FROM blogs b JOIN myMembers m ON (b.memberId = m.memberId) ";
...
// 총 게시물 수 계산
$totalSql = "SELECT COUNT(*) FROM blogs b JOIN myMembers m ON (b.memberId = m.memberId) " . $where;
...
<div class="right board__select">
    <form action="cummunitySearch.php" name="boardSearch" method="get">
        <fieldset>
            <input type="search" name="searchKeyword" id="searchKeyword" placeholder="검색어를 입력하세요!" value="<?= isset($searchKeyword) ? $searchKeyword : '' ?>" required>
            <select name="searchOption" id="searchOption">
                <option value="title" <?= (isset($searchOption) && $searchOption == 'title') ? 'selected' : '' ?>>제목</option>
                <option value="content" <?= (isset($searchOption) && $searchOption == 'content') ? 'selected' : '' ?>>내용</option>
                <option value="name" <?= (isset($searchOption) && $searchOption == 'name') ? 'selected' : '' ?>>등록자</option>
            </select>
